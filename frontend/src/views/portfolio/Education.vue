...
<script>
  ...
  let url=store.state.resourceHost; //서버주소 api
    export default {
      data(){return{
      myeducation:[{}],
      fields:[{key:'title',label:'교육명'},{key:'agency',label:'교육기관'},{key:'score',label:'점수'},{key:'start_date',label:'시작날짜'},{key:'end_date',label:'종료날짜'},{key:'edit',label:'마지막수정날짜'},{key:'편집',label:''}],
      edit:'1',
      }},
      components: {
      ...
      mounted(){
        this.educationView();
        // this.myeducation.edit=new Date().toJSON().slice(0,10).replace(/-/g,'.');
      },
      methods:{
        click(row){
          console.log(row)
        ...
        educationView(){
          axios.get(`${url}/education`)
                    ...
                    this.myeducation[i].edit=new Date().toJSON().slice(0,10).replace(/-/g,'.')
                    }
      ...
      deletepj(item){
            ...
            mvedit(){
              this.$router.push({path:'/portfolioModify',query:this.myeducation})
            }
            //상세페이지에 수정 기능 넣기
