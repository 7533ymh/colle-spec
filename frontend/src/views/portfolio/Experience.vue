...
<!--
 여기에 자격증  넣으셈
-->
      <b-table responsive="sm" striped :fields="fields" hover :items="myexperience" v-if="show" @row-clicked="click" >                        
  ...
  let url=store.state.resourceHost; //서버주소 api
    export default {
      data(){return{
          show:false,
      myexperience:[{}],
      fields:[{key:'country',label:'나라'},{key:'start_date',label:'시작날짜'},{key:'end_date',label:'종료날짜'},{key:'edit',label:'작성일'},{key:'편집',label:''}],
      edit:'1',
      }},
      components: {
      ...
      methods:{
        ...
        experienceView(){
          ...
          axios.get(`${url}/experience`)
                    ...
                    // this.myexperience[i].edit=this.myexperience[i].edit.slice(0,10).replace(/-/g,'.');
                    }
                    this.show=true
        })
      },
